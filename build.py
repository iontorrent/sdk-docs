...
##Global Settings
TORRENT_SERVER_BASE_URL = "http://ioneast.ite"

##API Docs Settings

API_BASE_URL = urlparse.urljoin(TORRENT_SERVER_BASE_URL, "/rundb/api/v1/")
...
DATABASE_DOCS_BASE_URL = urlparse.urljoin(TORRENT_SERVER_BASE_URL, "/admin/doc/models/")
...
main_schema_request = requests.get(API_BASE_URL, params={"format": "json"})
...
for resource_name, resource_values in main_schema_request.json().items():
    if resource_name not in EXCLUDE_RESOURCES:
        ...
        single_resource_schema_request = requests.get(urlparse.urljoin(API_BASE_URL, resource_values["schema"]), params={"format": "json"})
        ...
        single_resource_demo_request = requests.get(urlparse.urljoin(API_BASE_URL, resource_values["list_endpoint"])+DEMO_URL_ARGS)
        single_resource_demo_request.raise_for_status()

        #Make a python demo
        python_demo = PYTHON_READ_EXAMPLE.format(request_url=urlparse.urljoin(DEMO_API_BASE_URL, resource_values["list_endpoint"]), resource_name=resource_name)
        ...
        with open(resource_doc_path, "w+") as output_file:
            ...
            for field, values in single_resource_schema_request.json()["fields"].iteritems():
                row = [ "**" + field + "**"]
                for col in cols:
                    if col == "default" and values[col] == "No default provided.":  values[col] = "n/a"
                    if col == "default" and values[col] == "No default provided.":  values[col] = "n/a"
                    if str(values[col]) == "False" :  values[col] = "false"
                    if str(values[col]) == "True"  :  values[col] = "true"
...
database_doc_scraper = ModelDocScraper(DATABASE_DOCS_BASE_URL)
